...
const handleRegister = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      message:
        'Heslo musí obsahovať aspoň jedno veľké písmeno, jedno malé písmeno, jednu číslicu a jeden špeciálny znak.',
    });
  }
...
const handleLogin = async (req, res) => {
  if (await logy.getNumberOfWrongLogins(req.body.ip)) {
    return res.status(401).json({
      message:
        'Príliš veľa neúspešných pokusov o prihlásenie z tejto IP adresy. Skúste to prosím neskôr.',
    });
  }
  const logbodyFailed = {
    ...req.body,
  ...
  if (match == true) {
    ...
    res
      .status(409)
      .json({ message: 'Používateľské meno alebo heslo je neplatné' });
  }
};
